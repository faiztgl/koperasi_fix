...
<body>  
      ...
      <table class="table table-bordered table-striped table-condensed">
        ...
        while ( $data=mysql_fetch_array($query)) : ?>
          <tr>
           ...
           <?php $haha= mysql_fetch_array(mysql_query("SELECT status from t_pinjam where kode_anggota='$d'")); ?>
           <?php if ($haha['status'] == 'belum lunas'): ?>
            <td><a class="btn btn-danger btn-xs" disabled="disabled" href="index.php?pilih=1.3&aksi=viewambil&kode_tabungan=<?php echo $data['kode_tabungan'];?>&kode_anggota=<?php echo $d;?>"><i class="fa fa-dollar"></i> Ada Hutang</a></td>
            <?php elseif($data['besar_tabungan'] == 0): ?>
              <td><a class="btn btn-danger btn-xs" href="index.php?pilih=1.3&aksi=viewambil&kode_tabungan=<?php echo $data['kode_tabungan'];?>&kode_anggota=<?php echo $d;?>"><i class="fa fa-dollar"></i> Tidak Aktif</a></td>
              <?php else: ?>
                <?php $sta=mysql_fetch_array(mysql_query("SELECT status from t_anggota where Kode_anggota='$d'")); ?>
                <td><a class="btn btn-danger btn-xs" href="index.php?pilih=1.3&aksi=viewambil&kode_tabungan=<?php echo $data['kode_tabungan'];?>&kode_anggota=<?php echo $d;?>&status=<?php echo $sta['status']; ?>"><i class="fa fa-dollar"></i> Ambil Uang</a></td>
              <?php endif ?>
            </tr> 
          <?php endwhile ?>
          <?php $i++ ?>
          <tr  class="info"><td colspan="5" align="center">Total</td>
            <td colspan="2">Rp. <?php $bu=mysql_fetch_array(mysql_query("SELECT sum(besar_tabungan) as besar_tab from t_tabungan")); echo number_format($bu['besar_tab']);
            echo '</td>';?>
         ...
         <h4 class="mb"><span class='glyphicon glyphicon-briefcase'></span> Data Pengambilan Uang "<?php echo $nama['nama_anggota']; ?>" </span>
           ...
           <span style="float:right;">
            <form action="anggota/proses_tabungan.php" method="get">
              ...
              <input type="hidden" value="<?php echo $lo; ?>" name="kode_tabungan"> <input type="hidden" value="<?php echo $luy; ?>" name="kode_anggota"> <input type="hidden" name="kode_simpan" value="<?php echo $ks; ?>"> 
            <!--   <input type="hidden" name="status" value="<?php echo $sta; ?>"> -->
              <?php $rtyu=mysql_fetch_array(mysql_query("SELECT *FROM t_tabungan where kode_tabungan='$lo' and kode_anggota='$luy'"));?>
            ...
            <?php $tab=mysql_fetch_array(mysql_query("SELECT besar_tabungan from t_tabungan where kode_anggota='$luy'")); ?>

            <input type="text" readonly="" value="<?php echo $tab['besar_tabungan']; ?>" placeholder="ambil uang" style="width:150px;height:30px;" id="txt2" onkeyup="sum();" placeholder="" onkeypress="return isNumberKey(event)" name="besar_ambil"/>
            ...
            <?php if ($tab['besar_tabungan'] == 0) :?>
              <button class="btn btn-danger" disabled="disabled"><i class="fa fa-dollar"></i> Ambil Uang</button>
              <?php else: ?>
                <button class="btn btn-danger"><i class="fa fa-dollar"></i> Ambil Uang</button>
              <?php endif ?>
            <a class="btn btn-primary" href="laporan/print_ambil.php?kode_anggota=<?php echo $luy; ?>">Print</a>
            </form>
          </span>
        </h4>
        <form class="form-inline" role="form">
  ...
  <tr  class="info"><td colspan="5"a align="center">Total</td>
    <td colspan="2">Rp. <?php $bu=mysql_fetch_array(mysql_query("SELECT sum(besar_ambil) as besar_ambil from t_pengambilan where kode_anggota='$luy'")); echo number_format($bu['besar_ambil']);
    echo '</td>';?>
  </tr>
