<?php
	...
	$jml_saham=$_POST['jml_saham'];
	$besar_tabungan=str_replace(".","",$_POST['besar_tabungan']);
	...
	switch ($pros){
		case "tambah" :
			...
			$qu=mysql_query("INSERT INTO t_tabungan VALUES('','$kode_anggota','$tgl_entri','$besar_tabungan')");
			$f=mysql_query("INSERT INTO t_simpan values('','$kode_tabungan','pokok','$besar_tabungan','$kode_anggota','$u_entry','$tgl_entri','$tgl_entri')");
			$que=mysql_query("SELECT max(kode_tabungan) AS a FROM t_tabungan");
			$dt = mysql_fetch_array($que);

				mysql_query("INSERT INTO t_anggota values('$kode_anggota','$dt[a]','$jml_saham','$besar_tabungan','$nama_anggota','$alamat_anggota','$jenis_kelamin','$pekerjaan','$tgl_masuk','$telp','$tempat_lahir','$tgl_lahir','aktif','$u_entry','$tgl_entri')");?>
				<script>
